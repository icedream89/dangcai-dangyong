...
const userStore = {
  ...
  init() {
    try {
      const userInfo = common_vendor.index.getStorageSync(USER_KEY);
      if (userInfo) {
        this.state.userInfo = JSON.parse(userInfo);
        this.state.isLoggedIn = true;
      }
    } catch (e) {
      common_vendor.index.__f__("error", "at store/user.js:19", "初始化用户状态失败", e);
    }
  },
  // 登录
  login(userInfo) {
    ...
    this.state.isLoggedIn = true;
    try {
      common_vendor.index.setStorageSync(USER_KEY, JSON.stringify(userInfo));
    } catch (e) {
      common_vendor.index.__f__("error", "at store/user.js:30", "保存用户信息失败", e);
    }
  },
  // 登出
  logout() {
    ...
    try {
      ...
      common_vendor.index.__f__("error", "at store/user.js:41", "清除用户信息失败", e);
    }
  },
  // 更新用户信息
  updateUserInfo(newInfo) {
    if (this.state.userInfo) {
      ...
      try {
        common_vendor.index.setStorageSync(USER_KEY, JSON.stringify(this.state.userInfo));
      } catch (e) {
        common_vendor.index.__f__("error", "at store/user.js:52", "更新用户信息失败", e);
      }
    }
  },
  // 判断是否为企业用户
